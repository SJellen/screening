...
function useTvTrending() {
    const {tvTrending, posterPath, handleTvClick, handleRibbon, ribbonToggle}  = useContext(Context)
    ...
    const tvTrendingTile = tvTrending.map((show, index) => 
        ...
        { ribbonToggle === show.id ?
         <i className="im im-bookmark im-bookmark-slider" onClick={() => handleRibbon(index, tvTrending)} style={{color: "#E1B517"}}></i> :
         <i className="im im-bookmark im-bookmark-slider" onClick={() => handleRibbon(index, tvTrending)} style={{color: ""}}></i>
       }
        </span>
