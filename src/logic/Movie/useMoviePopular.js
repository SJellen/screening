...
function useMoviePopular() {
    const {posterPath, popularMovies, handleMovieClick, handleRibbon, ribbonToggle}  = useContext(Context)
    ...
    const popularMoviesTile = popularMovies.map((movie, index) => 
        ...
        { ribbonToggle === movie.id ?
         <i className="im im-bookmark im-bookmark-slider" onClick={() => handleRibbon(index, popularMovies)} style={{color: "#E1B517"}}></i> :
         <i className="im im-bookmark im-bookmark-slider" onClick={() => handleRibbon(index, popularMovies)} style={{color: ""}}></i>
       }
        </span>
