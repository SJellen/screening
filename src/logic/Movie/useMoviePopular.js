...
function useMoviePopular() {
    ...
    const popularMoviesTile = popularMovies.map((movie, index) => 
        ...
        <Link to={`/itemPage/${movie.id}`}><img  src={movie.poster_path !== null ? `${posterPath}${movie.poster_path}`: blankSquare} alt="poster" onClick={() => handleMovieClick(index, popularMovies)}/></Link>
