...
function useSearchResultsMovie() {
    ...
    const searchResultsMovieTile = searchResultsMovie.map((movie, index) => 
        ...
        <Link to={`/itemPage/${movie.id}`}><img  src={movie.poster_path !== null ? `${posterPath}${movie.poster_path}` : blankSquare} alt="poster" className="slider-poster" onClick={() => handleMovieClick(index, searchResultsMovie, true)}/></Link>
