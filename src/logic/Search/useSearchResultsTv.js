...
function useSearchResultsTv() {
    ...
    const searchResultsTvTile = searchResultsTv.map((show, index) => 
        ...
        <Link to={`/itemPage/${show.id}`}><img  src={show.poster_path !== null ? `${posterPath}${show.poster_path}` : blankSquare} alt="poster" onClick={() => handleTvClick(index, searchResultsTv, true)}/></Link>
