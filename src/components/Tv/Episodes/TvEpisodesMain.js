...
function TvEpisodesMain() {
    ...
    const {tvDetails, posterPath, dateSplitter, handleSeasonClick, tvSeason}  = useContext(Context)
    ...
    const tvEpisodesTile =  tvDetails.seasons && tvDetails.seasons.map((show) => 
        <div className="detail-season-number-box" key={show.id}>
           <Link to={`/tvPage/${tvDetails.id}/${tvSeason}`} style={{ textDecoration: 'none', color: "white"  }}>
               <span onClick={(e) => handleSeasonClick(e, show.season_number)} className="detail-season-number">{show.season_number === 0 ? "Specials" : show.season_number}</span>
           </Link> 
        </div>
    ...
    const lastEpisodeTile = ( lastEpisode && 
        ...
        <div className="details-episode-card-img-container">
            <img  src={lastEpisode.still_path !== null ? `${posterPath}${lastEpisode.still_path}` : episodePlaceholder} alt="poster" className="details-episode-card-img" /> 
        </div>
    ...
    const nextEpisodeTile = ( nextEpisode && 
        ...
        <div className="details-episode-card-img-container">
            <img  src={nextEpisode.still_path !== null ?  `${posterPath}${nextEpisode.still_path}` : episodePlaceholder} alt="poster" className="details-episode-card-img" />
        </div>
