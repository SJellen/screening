...
function TvEpisodesMain() {
  const { tvDetails, handleSeasonClick, tvSeason, setTvSeason } =
    useContext(Context);

  const tvEpisodesTile =
    tvDetails.seasons &&
    tvDetails.seasons.map((show) => (
      <div className="detail-season-number-box" key={show.id}>
        <Link
          to={`/tvPage/${tvDetails.id}/${tvSeason}`}
          style={{ textDecoration: "none", color: "white" }}
        >
          <span
            onClick={(e) => handleSeasonClick(show.season_number)}
