...
function TvRecommendations() {
    ...
    const {posterPath, handleTvClick, tvRecommendationArr, handleRibbonTv, watchListTv, ratingTruncate, watchListTvIds, setWatchListTv}  = useContext(Context)
    ...
    { watchListTvIds.includes(show.id) ?
         <i className="im im-bookmark im-bookmark-slider" onClick={() => handleRibbonMovieRecommendations(index, tvRecommendationArr)} style={{color: "#E1B517"}}></i> :
         <i className="im im-bookmark im-bookmark-slider" onClick={() => handleRibbonMovieRecommendations(index, tvRecommendationArr)} style={{color: ""}}></i>
       }
        </span>
