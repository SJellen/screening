...
function TvDetails() {
    ...
    const networkList = networks && networks.map(company => 
        <div key={company.id}>
    
            <li className="movie-details-list-item"><span>{company.logo_path !== null ? <img className="movie-details-list-logo" src={`${posterPathSmall}${company.logo_path}`} alt="company logo"/> : ''}</span>{company.name}</li> 
        </div>)  
        
    const creatorList = created_by && created_by.map(company => 
            <div key={company.id}>
        
                <li className="movie-details-list-item"><span>{company.profile_path !== null ? <img className="movie-details-list-logo" src={`${posterPathSmall}${company.profile_path}`} alt="company logo"/> : ''}</span>{company.name}</li> 
            </div>)     
