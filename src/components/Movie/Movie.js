...
function Movie() {
  ...
  const releaseStatus =
    movieDetails.status === "Released" ? "Released" : "Release date";
  ...
  function handleRibbonMoviePage(arr) {
    ...
    if (watchListMovieIds && watchListMovieIds.includes(selection)) {
      setWatchListMovie([
        ...watchListMovie.filter((item) => item.id !== selection),
      ]);
    } else {
    ...
    <div className="details-container">
          ...
          <img
            src={
              movieDetails.poster_path !== null
                ? `${posterPathLarge}${movieDetails.poster_path}`
                : blankSquare
            }
            alt="movie poster"
        ...
        <div className="detail-word-box">
          <div className="details-top-word-box">
            ...
            watchListMovieIds.includes(movieDetails.id) ? (
              <BookmarkIcon
                ...
                fontSize="x-large"
                onClick={() => handleRibbonMoviePage(movieDetails)}
                style={{ color: "#E1B517" }}
              />
            ) : (
              <BookmarkAddIcon
          ...
          <div className="details-mid-word-box">
            <span className="details-release">
              {releaseStatus} {dateSplitter(movieDetails.release_date)}
            </span>
            {movieDetails.runtime > 0 ? (
              <span className="details-runtime">
                Runtime {timeConverter(movieDetails.runtime)}
              </span>
            ) : (
              ""
            )}
      ...
      <div className="details-slider-video-watch-container">
        ...
        {movieLoaded && movieVideoTile && movieVideoTile.length > 0 ? (
          <div>
            ...
            <div className="details-video-container">{movieVideoTile}</div>
          </div>
        ) : (
          ""
        )}
      ...
      {castMovieTile && castMovieTile.length > 0 ? (
        <div className="details-slider-container">
          ...
          <div className="details-cast-slider">{castMovieTile}</div>
        </div>
      ) : (
        ""
      )}
      ...
      {crewMovieTile && crewMovieTile.length > 0 ? (
        <div className="details-slider-container">
          ...
          <div className="details-cast-slider">{crewMovieTile}</div>
        </div>
      ) : (
        ""
      )}

      {movieRecommendTile.length > 0 ? (
        <div className="details-slider-container">
          <span className="details-slider-title">Recommendations</span>
          <div className="details-cast-slider">{movieRecommendTile}</div>
        </div>
      ) : (
        ""
      )}

      {movieSimilarTile.length > 0 ? (
        <div className="details-slider-container">
          <span className="details-slider-title">Similar Movies</span>
          <div className="details-cast-slider">{movieSimilarTile}</div>
        </div>
      ) : (
        ""
      )}
      ...
      {movieReviewTile.length > 0 ? (
        <div>
          ...
          <div className="details-review-container">{movieReviewTile}</div>
        </div>
      ) : (
        ""
      )}
